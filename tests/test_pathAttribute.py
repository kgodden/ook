...
regex2 = '(?P<experiment>\w+)/(?P<camera>\w+)/(?P<channel>\w+)/(?P<dir>\d+)/image_D(?P<timestamp>.*)Z_(?P<frame>\d{1}).*'
path2 = 'sync_test/camera1/structured_light/0000/image_D2015-10-27T13-26-52-302857Z_9.jpg'
...
class TestPathAttribute(TestCase):
    ...
    def test_evaluate(self):
        ...
        self.assertEquals(values, ['sync_test', 'camera1', 'structured_light', '0000', '2015-10-27T13-26-52-302857', '9'])

        # test with timestamp as a fractional seconds number
        attribute = ook.PathAttribute(regex2,
                                      {'timestamp': lambda v: ook.to_timestamp(datetime.strptime(v, '%Y-%m-%dT%H-%M-%S-%f'))}
                                      )

        values = attribute.evaluate(path2)
        self.assertEquals(values, ['sync_test', 'camera1', 'structured_light', '0000', '1445952412.302857', '9'])
